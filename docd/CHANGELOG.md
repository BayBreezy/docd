...
- 5ddb2c5: Fix package-manager command snippets so they are expanded before Nuxt Content parses markdown. This routes `prose-pm-*` output through the same Shiki highlighting pipeline as normal fenced code blocks, which keeps snippets highlighted consistently after client-side navigation and hard reloads.
...
- b38d997: Add `body.maxWidth` app-config option for controlling page content width
  - Add `NuxtAnnouncer` to the root app template.
  - Replace `ScrollArea` with a lighter `scroll-fade` utility in the docs layout and mobile nav.
  - Refactor the page transition route watcher.
  - Bump dependencies (Nuxt, Tailwind CSS, nuxt-component-meta, and related packages).

## 0.2.0

### Minor Changes

- 7bda1b0: Add `docd.css.extend` config option to merge consuming-app CSS files into the layer's Tailwind build
